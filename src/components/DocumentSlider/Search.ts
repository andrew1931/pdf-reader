...
const renderSearchList = (list: HTMLUListElement, listItems: DocumentText, searchVal: RegExp) => {
   list.innerHTML = '';
   if (listItems.length === 0) {
      const emptyResult = document.createElement('div');
      emptyResult.classList.add('w-full', 'text-sm', 'font-semibold', 'text-center', 'mt-4');
   ...
   listItems.forEach((el) => {
      ...
      text.classList.add('text-xs', 'w-10/12', 'break-words');
      text.innerHTML =
         'page ' +
         `<b>${el.pageNumber}</b>` +
         ': ' +
